...
function btnAddImg_onclick() {

	if(window.event){  // IE
		var newOpt;
		newOpt = document.createElement("OPTION");
		newOpt = new Option("[Text]" + txtImgText.value, "[Text]" + txtImgText.value);
		//newOpt.id = txtImg.value;
		lstUrls.add(newOpt);
	} else {  // Netscape/Firefox/Opera
		lstUrls.innerHTML = lstUrls.innerHTML + "<OPTION>[Text]" + txtImgText.value + "</OPTION>";
	}
...
for(i=0; i<lb.length; i++)  {
	if(Mod(i+1,cols) == 1 || cols==1){
		...
		if(cmbPos.selectedIndex==3){
			...
			for(j=0; j<cols; j++)  {
				if(i+(j+1)<=lb.length)
					tmpStr = tmpStr + "<td " + tmpAlignText + tmpCellBgColor + "><font size=1 " + tmpFontColor + ">" + /*txtOptText.value +*/ (tmpStart+i+(j+1)) + ". " + tmpOptStr[1] + "</font></td>";
			}
			tmpStr = tmpStr + "</tr>";
		}
	...
	if(cmbPos.selectedIndex==1)
		tmpStr = tmpStr + "<td valign=\"bottom\" " + tmpCellBgColor + "><font size=1 " + tmpFontColor + ">" + /*txtOptText.value +*/ (tmpStart+i+1) + ". " + tmpOptStr[1] + "</font></td>";


	//Si lleva link
	if(chkThumb.checked==true){
		if(chkLink.checked==true){
			tmpLinkI = "<a href=\"" + tmpOptStr[0] + "\" target=blank_>";
			tmpLinkF = "</a>";
		}
	...
	tmpStr = tmpStr + "<td>" + tmpLinkI + "<img src=\"" + tmpOptStr[0] + "\"" + tmpWidth + tmpImgStyle + ">" + tmpLinkF + "</td>";
	...
	if(Mod(i+1,cols) == 0){
		...
		if(cmbPos.selectedIndex==2){
			tmpStr = tmpStr + "<tr>";
			for(j=1; j<=cols; j++)  {
				tmpStr = tmpStr + "<td " + tmpAlignText + tmpCellBgColor + "><font size=1 " + tmpFontColor + ">" + /*txtOptText.value +*/ (tmpStart+i-cols+1+j) + ". " + tmpOptStr[1] + "</font></td>";
			}
			tmpStr =  tmpStr + "</tr>";
		}
	}
...
if(fin==0){
	...
	if(cmbPos.selectedIndex==2){
		...
		for(i=0; i<cant; i++)  {
			tmpStr = tmpStr + "<td align=\"center\" " + tmpCellBgColor + "><font size=1 " + tmpFontColor + ">" + /*txtOptText.value +*/ (tmpStart+rows*cols-cols+i+1) + ". " + tmpOptStr[1] + "</font></td>";
		}
		tmpStr =  tmpStr + "</tr>";
	}
}
